...
async def ingest_telemetry_api(telemetry_chunk: list):
    try:
        await app.state.redis_pool.enqueue_job("process_train_telemetry", telemetry_chunk)
        return {"status": "enqueued", "count": len(telemetry_chunk)}
    except Exception as e:
