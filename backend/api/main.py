...
async def ingest_telemetry_api(request: TelemetryIngestRequest):
    ...
    # We will use 'process_train_telemetry' as the instruction says
    await redis_pool.enqueue_job("process_train_telemetry", request.train_numbers)
    return {"status": "enqueued", "train_count": len(request.train_numbers)}
