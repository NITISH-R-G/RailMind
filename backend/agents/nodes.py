...
async def reason_node(state: AgentState) -> AgentState:
    try:
        ...
        await log_agent("reason_node", f"[RAILMIND] Contacting AI to reason about {len(anomalies)} anomalies using tool-calling loop...")
        ...
        # Inject standard parsed data mimicking former tool dispatches to avoid disrupting UI
        try:
            # Reconstruct legacy nested structure for UI compat or keep flat
...
async def reroute_node(state: AgentState) -> AgentState:
    try:
        ...
        if anomalies:
            ...
            lat = anomaly.get("lat")
            lng = anomaly.get("lng")
            await log_agent("reroute_node", f"[RAILMIND] Evaluating geo-coordinates (lat: {lat}, lng: {lng}) for track availability matrices...")
...
async def supervisor_node(state: AgentState) -> dict:
    try:
        ...
        # Determine next node dynamically based on state rather than hardcoded sequence
        if not state.get("raw_train_data") and not state.get("anomalies"):
