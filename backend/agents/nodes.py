...
async def reason_node(state: AgentState) -> dict:
    try:
        ...
        from ..services.ai_service import reason_with_ai
        await log_agent("THINKING", "Executing autonomous tool-calling loop...")
        ...
        return {
            "claude_reasoning": json.dumps(plan),
            "reroute_plan": plan.get("reroute_plan"),
            "ai_latency_ms": latency,
            "errors": ["CLEAR"] # Reset errors since we just handled them
        }
...
async def reroute_node(state: AgentState) -> dict:
    try:
        ...
        if anomalies:
            ...
            await log_agent("reroute_node", f"[RAILMIND] Evaluating geo-coordinates (lat: {lat}, lng: {lng}) against dynamic programming track availability matrices...")

            # Track availability evaluation via coordinates
            track_available = True
            if lat > 26.0 and lng > 80.0:
                # Mock evaluation: northern tracks restricted based on coordinates
                track_available = False
                await log_agent("reroute_node", f"[RAILMIND] Track matrix unavailable for direct route due to coordinate bounding box. Bypassing anomaly location.")

            blocked = anomaly.get("location") or start_station

            # Use Fallback DP route discovery tool (dijkstra is used per AGENTS.md mandate)
            result = dijkstra_route_discovery(start_station, target_station, blocked_station=blocked if not track_available else None)
...
async def supervisor_node(state: AgentState) -> dict:
    try:
        ...
        # Self correction loop check: feedback errors to the reason_node
        try:
            ...
            if "Kanpur" in maintenance and "restricted" in maintenance.lower():
                 await log_agent("supervisor_node", "[RAILMIND] [WARNING] Conflict detected in maintenance task. Re-routing to Reasoner for self-correction.")
                 return {
