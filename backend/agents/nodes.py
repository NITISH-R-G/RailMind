...
async def evaluate_previous_action(state: AgentState) -> dict:
    try:
        ...
        if not state.get("raw_train_data"):
            ...
            return {"raw_train_data": live_trains, "last_api_call": datetime.utcnow().isoformat(), "railways_latency_ms": int((time.time() - start_time) * 1000)}
...
async def reason_node(state: AgentState) -> dict:
    try:
        ...
        # Use reason_with_ai from ai_service.py for autonomous tool-calling loop
        errors = state.get("errors", [])
        
        # Clear errors if they contained CLEAR token
        if errors and errors[0] == "CLEAR":
            errors = []

        mitigation_plan = await reason_with_ai(anomalies, errors)
        
        latency = int((time.time() - start_time) * 1000)
        await log_agent("reason_node", f"[RAILMIND] Real Autonomous Brain cycle complete ({latency}ms)")
        
        # Generate department tasks from the structured mitigation plan
        department_tasks = [
            {
                "department": "maintenance",
                "task_description": mitigation_plan.get("maintenance_task", ""),
                "urgency": "high",
                "action_required": "Immediate"
            },
            {
                "department": "operations",
                "task_description": mitigation_plan.get("operations_task", ""),
                "urgency": "high",
                "action_required": "Immediate"
            },
            {
                "department": "station_manager",
                "task_description": mitigation_plan.get("station_manager_task", ""),
                "urgency": "medium",
                "action_required": "Broadcast"
            }
        ]

        return {
            "claude_reasoning": json.dumps(mitigation_plan),
            "reroute_plan": mitigation_plan.get("reroute_plan"),
...
async def reroute_node(state: AgentState) -> dict:
    try:
        ...
        if anomalies:
            ...
            if not target_station:
                if start_station == "Kanpur Central":
                    target_station = "Varanasi"
                else:
                    target_station = "New Delhi" # generic fallback for dynamic routing

            # Evaluate geo-coordinates for DP route discovery fallback
            lat = anomaly.get("lat")
            lng = anomaly.get("lng")
            if lat and lng:
                await log_agent("reroute_node", f"[RAILMIND] Evaluating geo-coordinates (lat: {lat}, lng: {lng}) for track availability matrix...")
            ...
            if result["status"] == "Success":
                ...
                return {
                    "reroute_plan": state.get("reroute_plan") or f"No detour bypass available: {status_msg}",
                    "detour_route": []
                }
    except Exception as e:
...
async def supervisor_node(state: AgentState) -> dict:
    try:
        ...
        # Pre-execution: evaluation step to populate raw data if needed
        if not state.get("raw_train_data") and last_node != "evaluate_previous_action":
            return {"next_node": "evaluate_previous_action", "last_node_executed": "supervisor_node"}

        # Data ingestion
        if state.get("raw_train_data") and not state.get("last_api_call"):
            return {"next_node": "ingest_node", "last_node_executed": "supervisor_node"}

        anomalies = state.get("anomalies", [])

        # Detection phase
        if state.get("raw_train_data") and not anomalies and last_node != "detect_node":
            return {"next_node": "detect_node", "last_node_executed": "supervisor_node"}

        # Prediction Phase
        if anomalies and not state.get("prediction") and last_node != "predict_node":
