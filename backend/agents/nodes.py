...
async def evaluate_previous_action(state: AgentState) -> AgentState:
    try:
        ...
        # In the decoupled architecture, raw_train_data is injected by the ARQ worker.
        # We no longer block and fetch trains manually in a loop here.
        if not state.get("raw_train_data"):
            state["raw_train_data"] = []
...
async def ingest_node(state: AgentState) -> AgentState:
    try:
        ...
        # Raw train data should be passed directly via state in the decoupled architecture.
