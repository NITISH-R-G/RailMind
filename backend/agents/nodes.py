...
async def reason_node(state: AgentState) -> dict:
    try:
        ...
        # Use reason_with_ai to run autonomous tool-calling loop and output MitigationPlan
        plan = await reason_with_ai(anomalies, errors=errors)
        
        if plan:
            # Map Pydantic dictionary output to the appropriate state fields
            ...
            await log_agent("reason_node", "[RAILMIND] [ERROR] AI reasoning failed to return a plan")
            return {}
...
async def reroute_node(state: AgentState) -> dict:
    try:
        ...
        if anomalies:
            ...
            # Provide fallback destinations if none are provided
            if not target_station:
                if start_station == "Kanpur Central":
                    target_station = "Varanasi"
                else:
                    target_station = "New Delhi"  # default fallback
