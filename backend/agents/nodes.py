...
async def reason_node(state: AgentState) -> AgentState:
    try:
        ...
        try:
            ...
            logger.error(f"Error in ai_service reason_with_ai: {e}")
            from ..services.ai_service import generate_dynamic_fallback
            plan = generate_dynamic_fallback(anomalies[0], errors=errors)

        state["claude_reasoning"] = json.dumps(plan)
        state["reroute_plan"] = plan.get("reroute_plan")
        state["ai_latency_ms"] = int((time.time() - start_time) * 1000)
        
        # clear errors
        if errors and len(errors) > 0 and "errors" in state:
            state["errors"] = ["CLEAR"]

        await log_agent("reason_node", f"[RAILMIND] Real Autonomous Brain cycle complete ({state.get('ai_latency_ms', 0)}ms)")
        ...
        try:
            from ..services.ai_service import generate_dynamic_fallback
            anomaly = state.get("anomalies", [{}])[0]
            plan = generate_dynamic_fallback(anomaly)
            state["claude_reasoning"] = json.dumps(plan)
...
async def reroute_node(state: AgentState) -> AgentState:
    try:
        ...
        if anomalies:
            ...
            # Use a fallback destination if none provided
            if not target_station:
                if start_station == "Kanpur Central":
                    target_station = "Varanasi"
                else:
                    target_station = "Delhi"

            # Add geo-coordinate checking for DP route discovery fallback
            lat = anomaly.get("lat")
            lng = anomaly.get("lng")
