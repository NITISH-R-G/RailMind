...
async def reason_node(state: AgentState) -> AgentState:
    try:
        ...
        # Replace static loop with call to autonomous tool-calling AI agent
        errors = state.get("errors", [])
        
        await log_agent("THINKING", "Initiating Tool-Calling Reason Node...")
        
        mitigation_plan = await reason_with_ai(anomalies, errors=errors)
        
        state["claude_reasoning"] = json.dumps(mitigation_plan)
        
        # We also need to map the output parameters to state so the graph progresses
        state["reroute_plan"] = mitigation_plan.get("reroute_plan")
...
async def reroute_node(state: AgentState) -> AgentState:
    try:
        ...
        if anomalies:
            ...
            # If no target station is available in telemetry, use a default for dynamic pathfinding
            if not target_station:
                target_station = "Varanasi"

            # Always extract and log geo-coordinates for track availability matrix checking
            ...
            # Set blocked station to the current anomaly location to force bypass detour
            blocked = anomaly.get("location") or start_station

            # Use dynamic programming Dijkstra fallback for route discovery
            result = dijkstra_route_discovery(start_station, target_station, blocked_station=blocked)

            # If path not found due to blockage (e.g. source is blocked), try standard routing
...
async def supervisor_node(state: AgentState) -> dict:
    try:
        ...
        # 3. Reason/AI Mitigation Plan
        if not state.get("claude_reasoning") or state.get("claude_reasoning") == "{}":
            # Avoid ping-ponging indefinitely if AI failed multiple times
        ...
        # 3.5 Self correction loop check
        try:
        ...
        # 5. Coordination
        if not state.get("department_tasks") or state.get("department_tasks") == ["CLEAR"]:
            return {"next_node": "coordination_node", "last_node_executed": "supervisor_node"}

        # 6. Communication
        if not state.get("sms_alerts_sent") or state.get("sms_alerts_sent") == ["CLEAR"]:
