...
async def ingest_node(state: AgentState) -> AgentState:
    try:
        await log_agent("SCANNING", "Processing train telemetry...")

        # In the decoupled architecture, ARQ workers pass telemetry chunks directly
        # into the LangGraph AgentState via the raw_train_data field.
        live_trains = state.get("raw_train_data", [])
        
        if not live_trains:
            await log_agent("ingest_node", "[RAILMIND] No raw train data found in state. Skipping.")
            return state
