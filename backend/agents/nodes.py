...
async def ingest_node(state: AgentState) -> AgentState:
    try:
        await log_agent("SCANNING", "Processing ingested telemetry...")
        ...
        await log_agent("ingest_node", f"[RAILMIND] Ingested {len(live_trains)} trains from incoming chunks")
    except Exception as e:
...
async def report_node(state: AgentState) -> AgentState:
    try:
        ...
        incident_report = {
            ...
            "location_geo": {
                "type": "Point",
                "coordinates": [anomaly.get("lng", 0.0), anomaly.get("lat", 0.0)]
            }
        }
