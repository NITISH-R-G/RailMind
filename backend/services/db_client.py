...
class FallbackDB:
    ...
    async def has_recent_incident(self, train_number, minutes=2):
        # We rely entirely on DuplicateKeyError on insert via the timestamp_window bucket.
        # This function should always return False to pass execution to insert_incident.
        return False
    ...
    async def insert_incident(self, incident):
        ...
        if not self.use_fallback:
            try:
                # Create a 5-minute bucket for idempotent insertions
                ...
                if ts_str:
                    ...
                    incident["timestamp_window"] = bucket_ts.isoformat()
                else:
                    incident["timestamp_window"] = datetime.now().isoformat()
