...
class FallbackDB:
    ...
    async def init_indexes(self):
        try:
            # Enforce idempotency: unique compound index on train_number + 5-minute bucketed window
            await self.db["incidents"].create_index(
                [("train_number", ASCENDING), ("timestamp_window", ASCENDING)],
                unique=True
            )
            # Geospatial index for train positions
    ...
    async def has_recent_incident(self, train_number, minutes=2):
        # Deprecated logic. Idempotency is handled entirely by insert_incident and unique indexing.
        return False
